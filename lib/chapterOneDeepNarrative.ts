...
export function buildChapterOneDeepNarrative(appData: AppData): ChapterOneDeepResult {
  ...
  if (topHidden) {
    ...
    sections.push({
      ...
      body: [
        bridge,
        `${touCount > 0 ? HIDDEN_TEXT_ROOTED[cat] : HIDDEN_TEXT[cat]} ${stageList.join("·")}의 지지 속에 이 힘이 ${hits.length}군데 자리하고 있고, 그중 ${touCount}군데는 실제로 천간에도 같은 글자가 드러나 있어(투간·透干) 안에만 머물지 않고 겉으로도 힘을 발휘합니다.`,
        touCount === 0
          ? `다만 아직 어느 자리에도 같은 글자가 겉으로 드러나 있지는 않아서, 이 힘은 스스로도 자각하기 전까지는 잘 보이지 않는 편입니다.`
