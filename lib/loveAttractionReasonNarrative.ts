...
/**
 * 深化(2026-09, 승인된 확장) — 새 신호 없음. 이미 판정된 6갈래 branch를
 * "처음 끌림과 시간이 지난 뒤의 차이" / "이 끌림이 실제 관계에서 어떻게
 * 나타나는지"라는 다른 두 질문에 한 번씩 더 적용한다.
 */
const OVER_TIME_BY_BRANCH: Record<Branch, string> = {
  판정보류형: "처음 끌리는 이유가 매번 다르게 느껴지는 만큼, 시간이 지나도 '이래서 좋다'는 한 가지 이유로 정리되지 않는 경우가 많습니다. 그런데도 관계 자체는 계속 이어지는 경우가 있는데, 그건 이유가 하나로 고정되지 않아서일 뿐 마음이 얕아서가 아닙니다.",
  이중일치형: "처음의 끌림이 시간이 지나도 잘 옅어지지 않는 편입니다. 여러 이유가 동시에 겹쳐서 시작된 마음이라, 한 가지 계기로 식는 게 아니라 여러 갈래가 동시에 흔들려야 비로소 마음이 변하는 구조에 가깝습니다.",
  채워지는형: "처음엔 '이 사람과 있으면 편하다'는 감각으로 시작되지만, 시간이 지날수록 그 편안함이 없으면 오히려 허전하게 느껴질 만큼 자연스러운 일부가 되기 쉽습니다. 초반의 끌림보다 오래된 뒤의 의존이 더 크게 자리 잡는 유형입니다.",
  방향지지형: "처음엔 상대의 어떤 부분에 끌렸다기보다 '이 관계를 하고 있으면 내가 원하는 방향으로 가고 있다'는 감각이 먼저 옵니다. 시간이 지나도 상대 자체보다 그 관계가 만들어주는 방향성에 대한 신뢰가 유지되는 편입니다.",
  경쟁밀림형: "처음엔 이 부분에도 눈길이 가지만, 시간이 지날수록 결국 다른 이유가 더 크게 자리를 차지하게 됩니다. 이 부분이 완전히 사라지는 건 아니지만, 관계를 지탱하는 중심 이유로 오래 남지는 않는 편입니다.",
  후보아님형: "처음부터 강하게 끌리는 경험 자체가 드문 편이라, 시간이 지나며 여러 조건이 하나씩 맞아갈 때 비로소 '이 사람이구나' 하는 확신이 서기 쉽습니다. 초반의 미적지근함이 오래가는 관계의 신호가 아니라는 뜻은 아닙니다.",
};

const IN_PRACTICE_BY_BRANCH: Record<Branch, string> = {
  판정보류형: "실제 관계에서는 '왜 이 사람이 좋은지' 물어보면 매번 조금씩 다른 대답을 하게 될 수 있습니다. 그날그날 다르게 느껴지는 지점을 그대로 말해도 괜찮은 상대 앞에서 마음이 더 편해집니다.",
  이중일치형: "실제 관계에서는 여러 이유가 동시에 겹쳐서 오는 만큼, 상대의 여러 면을 한꺼번에 좋아한다고 표현하게 되기 쉽습니다. 상대 입장에서는 애정 표현이 유독 다층적이라고 느껴질 수 있습니다.",
  채워지는형: "실제 관계에서는 그 상대와 있을 때만 유독 편안해지는 티가 나기 쉽습니다. 평소라면 예민하게 반응할 일도 그 사람 앞에서는 무던하게 넘기는 모습을 상대가 먼저 알아차릴 수 있습니다.",
  방향지지형: "실제 관계에서는 상대가 무언가를 직접 해주지 않아도 그 존재만으로 힘을 얻는 모습을 보이기 쉽습니다. 상대는 '내가 딱히 해준 게 없는데 왜 저렇게 의지할까' 하고 의아해할 수 있지만, 그 자체가 이 관계의 힘입니다.",
  경쟁밀림형: "실제 관계에서는 이 부분이 관계의 결정적인 이유로 언급되는 일은 드뭅니다. 오히려 다른 이유를 설명하다가 '아, 이것도 있었지' 하고 뒤늦게 덧붙여지는 경우가 많습니다.",
  후보아님형: "실제 관계에서는 첫 만남의 강렬함보다, 여러 번 만나며 조건이 하나씩 맞아가는 과정 자체가 관계를 다지는 역할을 합니다. 상대에게도 '천천히 알아가고 싶다'는 태도로 비치기 쉽습니다.",
};
...
export function generateLoveAttractionReasonNarrative(appData: AppData, gender: "male" | "female"): LoveAttractionReasonNarrativeResult {
  ...
  return {
    paragraphs: [
      ...
      { text: OVER_TIME_BY_BRANCH[branch], sourceNote: `초반-오래된뒤 차이(branch=${branch})` },
      { text: IN_PRACTICE_BY_BRANCH[branch], sourceNote: `실제 관계 속 모습(branch=${branch})` },
