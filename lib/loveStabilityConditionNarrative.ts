...
/**
 * 深化(2026-09, 승인된 확장) — 새 신호 없음. 이미 판정된 일지 십성 10종을
 * "오래갈수록 중요해지는 상대의 태도" / "피해야 할 관계 구조"라는 다른
 * 두 질문에 한 번씩 더 적용한다. TYPE_TEXT의 scene이 이미 "지금 이
 * 순간의 편안함"을 다루므로, 여기서는 시간축(오래갈수록)과 구조축
 * (피해야 할 것)으로 시야를 넓힌다.
 */
const LONG_TERM_BY_TYPE: Record<DaySipseong, string> = {
  비견: "오래 갈수록 중요한 건 이벤트가 아니라 각자의 영역을 계속 존중해 주는 태도입니다. 초반엔 자유로움이 매력이지만, 시간이 지나며 그 자유가 서로에 대한 무관심으로 오해받지 않으려면 가끔은 의식적으로 서로의 안부를 챙기는 습관이 필요합니다.",
  겁재: "오래 갈수록 중요한 건 처음의 확실함이 계속 유지되는지입니다. 초반에 정확했던 몫과 역할이 시간이 지나며 흐지부지되면, 겉으로는 표 안 내도 신뢰가 서서히 깎여나갈 수 있습니다.",
  식신: "오래 갈수록 중요한 건 특별한 이벤트가 아니라 일상의 대화가 끊기지 않는 것입니다. 초반의 설렘이 잦아든 뒤에도 사소한 이야기를 계속 나눌 수 있는 상대인지가, 이 관계가 오래가는지를 가르는 지점이 됩니다.",
  상관: "오래 갈수록 중요한 건 하고 싶은 말을 계속 눌러야 하는 관계가 되지 않는 것입니다. 초반엔 맞춰주다가도, 표현을 계속 참아야 하는 시간이 길어지면 관계 자체에 지치기 쉽습니다.",
  편재: "오래 갈수록 중요한 건 계획이 틀어졌을 때 함께 움직여 줄 수 있는지입니다. 초반의 새로움이 사라진 뒤에도, 변화에 유연하게 반응해 주는 상대인지가 이 사람에게는 계속 중요하게 남습니다.",
  정재: "오래 갈수록 중요한 건 작은 약속들이 여전히 지켜지는지입니다. 초반의 강렬함보다 반복되는 신뢰가 이 사람에게는 훨씬 큰 의미를 갖기 때문에, 시간이 지나도 약속의 무게가 가벼워지지 않는 관계가 필요합니다.",
  편관: "오래 갈수록 중요한 건 힘든 상황에서도 책임을 회피하지 않는 모습이 꾸준히 이어지는지입니다. 초반의 확신이 시간이 지나며 흐려지면, 이 사람은 다시 마음을 놓기까지 오래 걸릴 수 있습니다.",
  정관: "오래 갈수록 중요한 건 정한 것을 지키는 꾸준함이 유지되는지입니다. 초반엔 감정 표현이 크지 않아도 넘어가지만, 시간이 지나며 약속이 자꾸 어긋나면 애정과 별개로 마음이 서서히 닫힐 수 있습니다.",
  편인: "오래 갈수록 중요한 건 서로의 공간을 침범하지 않는 거리감이 유지되는지입니다. 초반의 호기심 어린 관심이 시간이 지나도 똑같은 강도로 이어지면, 이 사람에게는 오히려 부담으로 자리 잡을 수 있습니다.",
  정인: "오래 갈수록 중요한 건 힘들 때 먼저 다가와 주는 태도가 계속 이어지는지입니다. 초반엔 괜찮은 척해도 넘어가지만, 그 괜찮은 척을 계속 곧이곧대로 믿고 방치하는 시간이 길어지면 서운함이 쌓입니다.",
};

const AVOID_STRUCTURE_BY_TYPE: Record<DaySipseong, string> = {
  비견: "반대로, 사소한 결정까지 매번 확인받으려 하고 각자의 시간을 인정하지 않는 관계는 이 사람을 빠르게 지치게 만듭니다.",
  겁재: "반대로, 비용이나 역할을 얼버무리고 넘어가면서 '좋은 게 좋은 거'라는 태도로 일관하는 관계는 이 사람에게 오래 맞지 않습니다.",
  식신: "반대로, 매번 특별한 이벤트로만 마음을 확인시키려 하고 평범한 일상을 시시하게 여기는 관계는 이 사람을 피곤하게 만들 수 있습니다.",
  상관: "반대로, 순간의 말과 감정 표현마다 정색하며 선을 긋는 관계는 이 사람이 스스로를 계속 검열하게 만들어 오래가기 어렵습니다.",
  편재: "반대로, 한 번 정한 방식을 절대 바꾸지 못하게 하고 변화 자체를 불편해하는 관계는 이 사람에게 부담으로 쌓이기 쉽습니다.",
  정재: "반대로, 약속이 별다른 설명 없이 자주 바뀌고 그걸 대수롭지 않게 여기는 관계는 이 사람의 마음을 서서히 물러서게 만듭니다.",
  편관: "반대로, 결정적인 순간마다 확실한 말을 피하고 책임을 미루는 관계는 이 사람의 신뢰를 조금씩 깎아냅니다.",
  정관: "반대로, 정해둔 것이 반복해서 어긋나고 그때마다 그럴듯한 이유만 대는 관계는 애정과 별개로 마음의 문을 닫게 만듭니다.",
  편인: "반대로, 궁금하다는 이유로 계속 캐묻고 혼자 있고 싶은 시간을 존중하지 않는 관계는 이 사람의 숨 쉴 공간을 줄어들게 만듭니다.",
